 ...
 */
static void send_message_to_mqtt(esp_mqtt_client_handle_t *client, QueueHandle_t* json_queue)
{
    int msg_id;
    ...
    if (json_queue != 0)
    {
        ...
        if (xQueueReceive(json_queue, &(msg), pdMS_TO_TICKS(1000)))
        {
            ...
            msg_id = esp_mqtt_client_publish(*client, "weather", msg.json, 0, 1, 0);
 ...
 * @return esp_err_t 
 */
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event,  QueueHandle_t json_queue)
{
